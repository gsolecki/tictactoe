	...
	public void init(String input) {
		...
		try {
			...
			for (int i = 0; i < noOfGames; i++) {
				...
				game.setPlayers(Arrays
						.asList(new Player[] { xPlayer, oPlayer }));
...
class TicTacToe {
	...
	public void play() {
		...
		do {
			...
			if (getCurrentState().equals(State.WON)
					|| getCurrentState().equals(State.DRAW)) {
				break;
			}
			changePlayer();
		} while (true);
...
class MiniMaxPlayer extends DefaultPlayer {
	...
	public static int evaluateLine(int row1, int col1, int row2, int col2,
			int row3, int col3, Board board, Player player) {
		int score = 0;

		if (board.getTile(row1, col1).getPlayer().equals(player)) {
			score = 1;
		} else if (!board.getTile(row1, col1).getPlayer()
				.equals(Player.NO_PLAYER)) {
			score = -1;
		}
		...
		} else if (!board.getTile(row2, col2).getPlayer()
				.equals(Player.NO_PLAYER)) {
			if (score == -1) {
				score = -10;
			} else if (score == 1) {
		...
		} else if (!board.getTile(row3, col3).getPlayer()
				.equals(Player.NO_PLAYER)) {
			if (score < 0) {
				score *= 10;
			} else if (score > 1) {
	...
	private Tile minimax(int depth, Player player) {
		...
		int bestScore = player.equals(this) ? Integer.MIN_VALUE
				: Integer.MAX_VALUE;
		int currentScore;
		Tile bestTile = null;
		...
		if (nextMoves.isEmpty() || depth == 0) {
			...
			for (Tile tile : nextMoves) {
				...
				currentScore = minimax(depth - 1,
						TicTacToe.getNextPlayer(player, players)).getScore();
...
class Board {
	...
	public List<Tile> getFreeTiles() {
		return getTiles().stream().filter(tile -> tile.isEmpty())
				.collect(Collectors.toList());
	}

	public Tile getTile(int row1, int col1) {
		return getTiles().stream()
				.filter(tile -> tile.equals(new Tile(row1, col1))).findFirst()
				.get();
	}
	...
	public void print(StringBuilder sb) {
		...
		for (int i = 0; i < getTiles().size(); i++) {
			...
			sb.append("|");
			sb.append(" ");
			sb.append(tile.getPlayer() != null ? String.valueOf(tile
					.getPlayer()) : " ");
			sb.append(" ");
		}
		printVertical(sb, -1);
	...
	public void setDim(int dim) {
		this.dim = dim;
		winningPatterns = Arrays
				.asList("|||------", "---|||---", "------|||", "|--|--|--",
						"-|--|--|-", "--|--|--|", "|---|---|", "--|-|-|--");
	}

	public void setTile(Tile newTile) {
		lastTile = getTiles().stream().filter(tile -> tile.equals(newTile))
				.findFirst().get();
	...
	public static String serialize(List<Tile> board, Player lastPlayer) {
		...
		board.stream().forEachOrdered(
				tile -> output.append(tile.getPlayer().equals(lastPlayer) ? "|"
						: "-"));
